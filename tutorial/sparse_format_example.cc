...
int main(int ac, char **av)
{
  ...
  std::cout << "Starting example COO_CSR_1" << std::endl;
  ...
  std::cout << "CSR relation inverse: " << r_csr_inv->prettyPrintString() << std::endl;
  Relation *r_coo_csr_comp = r_csr_inv->Compose(r_coo);
  std::cout << "Composition of CSR inverse and COO relations: " << r_coo_csr_comp->prettyPrintString() << std::endl;
  std::cout << std::endl;
  ...
  Relation *coo_inv = new Relation("{[i,j] -> [n2] : rowcol_inv(n2,0) = i && rowcol_inv(n2,1) = j "
                                   "&& 0 <= n2 && n2 < NNZ }");
  std::cout << "COO inverse relation: " << coo_inv->prettyPrintString() << std::endl;
  Relation *result = coo_inv->Compose(dense);
  std::cout << "Composition of COO inverse and dense relation: " << result->prettyPrintString() << std::endl;
  std::cout << std::endl;

  std::cout << "Starting example COO_BCSR" << std::endl;
  Relation *Flatten = new Relation("{ [n] -> [ti,tj,iz,jz,in,zn,riz,rjz,rin,rjn] :"
                                   " n = iz * 9 + jz }");
  std::cout << "Flatten relation: " << Flatten->prettyPrintString() << std::endl;
